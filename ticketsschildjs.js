...
const nameInput = document.getElementById('nameInput');
const fullNameCell = document.getElementById('fullNameCell');

// Retrieve the name from local storage, if available
const storedName = localStorage.getItem('fullName');
if (storedName) {
  nameInput.value = storedName;
  fullNameCell.textContent = storedName;
}

// Function to update the name in local storage and table cell
function updateName() {
...
// Retrieve the email from local storage, if available
const storedEmail = localStorage.getItem('email');
if (storedEmail) {
  emailInput.value = storedEmail;
  confirmEmailInput.value = storedEmail;
  emailCell.textContent = storedEmail;
}

// Function to update the email in local storage and table cell
function updateEmail() {
  const email = emailInput.value;
  localStorage.setItem('email', email);
  emailCell.textContent = email;
  
  // Enable submit button if email is entered and confirmed
  if (email === confirmEmailInput.value) {
    submitButton.disabled = false;
  } else {
    submitButton.disabled = true;
  }
...
// Add event listener to the email input fields
emailInput.addEventListener('input', updateEmail);
confirmEmailInput.addEventListener('input', updateEmail);